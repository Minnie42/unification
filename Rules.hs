...
rule1 sol ((BL [B x y], BL [B x' y']):gamma) = 
  Just (Just sol, ((V x, V x'):(V y, V y'):gamma))
rule1 sol _ = Nothing
...
rule3 sol ((V (Concrete x), V (Concrete y)):gamma)
  | x /= y = 
    Just (Nothing, (V (Concrete x), V (Concrete y)):gamma)
  | otherwise = Nothing
rule3 sol _ = Nothing
...
rule4 sol ((V (Meta x), V (Concrete y)):gamma) = 
  Just (
    Just ((Sub (Meta x) (Concrete y)):sol), 
    applySolEntryToGamma 
      (Sub (Meta x) (Concrete y)) 
      ((V (Meta x), V (Concrete y)):gamma)
  )
...
rule5 sol ((V (Meta x), V (Meta y)):gamma) 
  |x /= y = 
    Just (
      Just ((Sub (Meta x) (Meta y)):sol), 
      applySolEntryToGamma 
        (Sub (Meta x) (Meta y)) 
        ((V (Meta x), V (Meta y)):gamma)
    )
  |otherwise = Nothing
rule5 sol _ = Nothing
...
rule6 
  ...
  (
    (
      BL ((B lvar1 lvar2):leftBind2:leftBinds), 
      BL ((B rvar1 rvar2):rightBinds)
    ):gamma
  ) = 
  Just (
    rule6Iterate 
      ((B lvar1 lvar2):leftBind2:leftBinds) 
      [] 
      (B rvar1 rvar2) 
      rightBinds 
      sol 
...
rule6 
  ...
  (
    (
      BL ((B lvar1 lvar2):leftBinds), 
      BL ((B rvar1 rvar2):rightBind2:rightBinds)
    ):gamma
  ) = 
  Just (
    rule6Iterate 
      ((B lvar1 lvar2):leftBinds) 
      [] 
      (B rvar1 rvar2) 
      (rightBind2:rightBinds) 
      sol 
      gamma
  )
rule6 sol _ = Nothing
...
rule6Iterate 
  ...
  (
    ...
    :(
      BL (tail leftBind), 
      BL (rightInit ++ (rightNext:rightTail))
    )
    :gamma
  ):(
    rule6Iterate 
      leftBind 
      (rightInit ++ [rightCurrent]) 
      rightNext 
      rightTail 
      sol 
      gamma
...
rule7 sol ((BL (bind:binds), BL []):gamma) = 
  Just (Nothing, ((BL (bind:binds), BL []):gamma))
rule7 sol _ = Nothing

rule8 :: Sol -> Problem -> Maybe (Maybe Sol, Problem)
rule8 sol ((BL [], BL (bind:binds)):gamma) = 
  Just (Nothing, ((BL [], BL (bind:binds)):gamma))
rule8 sol _ = Nothing
...
rule10 
  ...
  (
    (
      BL ((CV name var1 var2):leftBinds), 
      BL rightBinds
    ):gamma
  ) = 
  Just [
...
rule11 
  ...
  (
    (
      BL leftBinds, 
      BL ((CV name var1 var2):rightBinds)
    ):gamma
  ) = 
  Just [
    (
