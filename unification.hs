...
recursiveUnification ((sol, problem):problems) sols 1
  ...
  | otherwise = recursiveUnification ((unJust next):problems) sols 2
  where
...
recursiveUnification ((sol, problem):problems) sols 6
  | next == Nothing = recursiveUnification ((sol, problem):problems) sols 3
  | otherwise = recursiveUnification ((unJust next)++problems) sols 3
  where
